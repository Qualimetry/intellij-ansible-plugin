...
/**
 * Persistent settings for the Qualimetry Ansible Analyzer plugin.
 * Stores per-rule enabled/disabled overrides and severity overrides.
 */
@State(name = "QualimetryAnsibleAnalyzerSettings", storages = @Storage("qualimetry-ansible.xml"))
public final class AnsibleAnalyzerSettings implements PersistentStateComponent<AnsibleAnalyzerSettings> {
    ...
    public static AnsibleAnalyzerSettings getInstance() {
        return ApplicationManager.getApplication().getService(AnsibleAnalyzerSettings.class);
    }

    @Override
    public AnsibleAnalyzerSettings getState() {
        return this;
    }

    @Override
    public void loadState(@NotNull AnsibleAnalyzerSettings state) {
