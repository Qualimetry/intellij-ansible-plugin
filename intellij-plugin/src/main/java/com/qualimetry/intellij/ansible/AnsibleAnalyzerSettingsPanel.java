...
/**
 * Settings panel for the Qualimetry Ansible Analyzer plugin.
 * Provides per-rule enable/disable, severity override, and filter.
 */
final class AnsibleAnalyzerSettingsPanel {
    ...
    AnsibleAnalyzerSettingsPanel() {
        enabledCheckBox = new JCheckBox("Enable Qualimetry Ansible Analyzer");
        ...
        importButton.addActionListener(e -> ImportFromSonarQubeAction.runImport(null, () ->
                loadFrom(AnsibleAnalyzerSettings.getInstance())));
    ...
    private void applyFilter(String filter) {
        ...
        for (RuleRow r : visible) {
            AnsibleAnalyzerSettings.RuleOverride override = settings.rules.get(r.key);
    ...
    void loadFrom(@NotNull AnsibleAnalyzerSettings settings) {
    ...
    void saveTo(@NotNull AnsibleAnalyzerSettings settings) {
        ...
        Map<String, AnsibleAnalyzerSettings.RuleOverride> existingRules = new java.util.HashMap<>(settings.rules);
        Map<String, AnsibleAnalyzerSettings.RuleOverride> rules = settings.rules;
        rules.clear();
        if (settings.rulesReplaceDefaults) {
            for (RuleRow r : allRows) {
                RowState state = tableState.get(r.key);
                AnsibleAnalyzerSettings.RuleOverride existing = existingRules.get(r.key);
                boolean enabled;
                String severity;
                if (state != null) {
                ...
                rules.put(r.key, withExistingParams(new AnsibleAnalyzerSettings.RuleOverride(enabled, severity), existing));
            }
        } else {
            for (RuleRow r : allRows) {
                RowState state = tableState.get(r.key);
    ...
    private static AnsibleAnalyzerSettings.RuleOverride withExistingParams(
            AnsibleAnalyzerSettings.RuleOverride next,
            AnsibleAnalyzerSettings.RuleOverride existing) {
